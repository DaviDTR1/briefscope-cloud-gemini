...
powered by Google Gemini (Gemini models + Google embeddings).
...
> This is the **CLOUD Gemini** variant. The embedding provider and the LLM are
> both fixed to Google, so it requires a Google API key and makes outbound
> requests to the Google Gemini API. For offline, no-key alternatives see the
...
```
client → Traefik (PathPrefix /api/briefscope_cloud_gemini) → FastAPI app
...
- **Vector store**: a bundled `chromadb/chroma` container
  (`briefscope_chroma_gemini`), reached over HTTP on the shared
  `queai_network`. Data persists in its own Docker volume.
- **Frontend**: a React (Vite + TypeScript) single-page app served from
  `frontend_dist/` at `/ui`.
- **LLM layer**: LangChain unified layer targeting `langchain-google-genai`.

Key REST routes (all under the plugin root path `/api/briefscope_cloud_gemini`):
...
- A **Google API key** with access to the Gemini API (entered later through the
  Settings UI).
- Outbound network access to the Google Gemini API.
...
Then open **Settings** in the UI and paste your Google API key. That's it — the
...
3. Find **BriefScope CLOUD Gemini** in the catalog and install it.
4. Open the plugin UI and go to **Settings** to enter your Google API key.

In this mode the kernel provides the `queai_network` and Traefik routes the app
at `/api/briefscope_cloud_gemini`; the base `docker-compose.yml` is used as-is
(no standalone override).

## Configuration

No secrets live in `.env`. The Google API key and runtime parameters are entered
...
|---|---|---|
| `ROOT_PATH` | `/api/briefscope_cloud_gemini` | FastAPI `root_path` / kernel path prefix. Blanked to `""` in `docker-compose.standalone.yml` so everything serves at the host root. |
| `LLM_MODE` | `cloud` | Selects the cloud provider path. Leave as-is. |

Tunable from the Settings UI:

- **Google API key** — required for the plugin to become "ready".
...
|---|---|---|
| **CLOUD Gemini** (this one) | Google Gemini + Google embeddings | Google key, outbound |
