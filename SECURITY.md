...
kernel. This policy covers the **BriefScope CLOUD Gemini** plugin only. For
...
- Vulnerabilities in the plugin backend (`app/`) — RAG, agents, file
  generation, configuration and the REST API under `/api/briefscope_cloud_gemini`.
...
## Notes specific to the CLOUD Gemini variant

This variant **requires a Google API key** and makes **outbound network
requests** to the Google Gemini API for both embeddings and chat completions.
This is by design. Operators should be aware that:
...
- Document content and prompts are sent to Google for embedding and generation.
